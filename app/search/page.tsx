...
export default function SearchPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {selectedCategory && (
                <span className="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800">
                  {selectedCategory}
                  <button onClick={() => setSelectedCategory(null)} className="ml-2 text-blue-600 hover:text-blue-500">
                    ×
                  </button>
                </span>
              )}
              {searchQuery && (
                <span className="inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-800">
                  "{searchQuery}"
                  <button onClick={() => setSearchQuery('')} className="ml-2 text-green-600 hover:text-green-500">
                    ×
                  </button>
                </span>
              )}
        ...
        {filteredAndSortedServices.length === 0 ? (
          <div className="py-16 text-center">
            <div className="mx-auto max-w-md">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">No encontramos servicios</h3>
              <p className="mt-2 text-gray-500">Intenta expandir el radio de búsqueda o cambiar los filtros.</p>
              <div className="mt-6">
                <Button
                  variant="outline"
                  onClick={() => {
      ...
      <AuthModal open={showAuthModal} onOpenChange={setShowAuthModal} defaultForm="login" />
    </div>
  );
}
